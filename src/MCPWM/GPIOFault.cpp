...
namespace esp::mcpwm {
    bool IRAM_ATTR _onFaultEnter(mcpwm_fault_handle_t faultHandle, const mcpwm_fault_event_data_t* faultData, void* userData) {
        auto& [fault, userInfo] = *reinterpret_cast<std::pair<GPIOFault*, void*>*>(userData);
        return static_cast<bool>(fault->_callbacks.onFaultEnter(*faultData, userInfo));
    }

    bool IRAM_ATTR _onFaultExit(mcpwm_fault_handle_t faultHandle, const mcpwm_fault_event_data_t* faultData, void* userData) {
        auto& [fault, userInfo] = *reinterpret_cast<std::pair<GPIOFault*, void*>*>(userData);
        return static_cast<bool>(fault->_callbacks.onFaultExit(*faultData, userInfo));
    }
}  // namespace esp::mcpwm
...
void GPIOFault::setCallbacks(const Callbacks& callbacks, void* userInfo, esp_err_t& err) {
    ...
    err = mcpwm_fault_register_event_callbacks(_fault, &faultCallbacks, &_userInfo);
    if (err != ESP_OK) {
